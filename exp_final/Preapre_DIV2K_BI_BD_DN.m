...
path_original = 'E:\\Userdata\\shenhao\\dataset';
dataset  = {'DIV2K'};
ext = {'*.jpg', '*.png', '*.bmp'};
...
for idx_set = 1:length(dataset)
    ...
    for idx_ext = 1:length(ext)
        filepaths = cat(1, filepaths, dir(fullfile(path_original, dataset{idx_set}, 'DIV2K_train_HR', ext{idx_ext})));
    end
    for idx_im = 1:length(filepaths)
        name_im = filepaths(idx_im).name;
        fprintf('%d. %s: ', idx_im, name_im);
        im_ori = imread(fullfile(path_original, dataset{idx_set}, 'DIV2K_train_HR', name_im));
        if size(im_ori, 3) == 1
        ...
        for scale = scale_all
            ...
            % 閽堝DIV2K鏂囦欢澶瑰拰benchmark鏂囦欢澶�
            folder_HR = fullfile('E:/Userdata/shenhao/dataset/DIV2K/DIV2K_train_HR');
            folder_LR = fullfile('E:/Userdata/shenhao/dataset/DIV2K/DIV2K_train_LR_noise', ['X', num2str(scale)]);
            if ~exist(folder_HR)
                mkdir(folder_HR)
            end
            if ~exist(folder_LR)
            ...
            % fn   DIV2K_train_LR_bicubic
            fn_LR = fullfile('E:/Userdata/shenhao/dataset/DIV2K/DIV2K_train_LR_noise', ['X', num2str(scale)], [name_im(1:end-4), 'x', num2str(scale), '.png']);
